...
function M.draw()
    ...
    -- Character-wide minimum mez level. Shown only when a mez spell is configured.
    local hasMez = false
    for _, e in ipairs(spells) do
        if spellutils.IsMezSpell(e) then hasMez = true; break end
    end
    if hasMez then
        ImGui.Text('Minimum mez level')
        if ImGui.IsItemHovered() then ImGui.SetTooltip(
            'Skip mezzing mobs below this level. 0 = no minimum (spell MaxLevel still applies).') end
        ImGui.SameLine()
        ImGui.SetNextItemWidth(NUMERIC_INPUT_WIDTH)
