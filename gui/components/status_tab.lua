...
local DO_FLAGS = {
    ...
    { key = 'antiAfk',  label = 'Anti-AFK', tt = 'Open/close a random bag (or inventory) after ~3–4 min of continuous true idle.' },
}
...
function M.drawControls()
    ...
    ImGui.SameLine()
    -- Burn button: start/stop a burn window. Debuff bands with a Burn phase cast during it.
    do
        ...
        if ImGui.IsItemHovered() then
            ImGui.SetTooltip(
                'Start/stop a burn window. Debuffs with a Burn band phase (Debuff tab) cast while it is active.\nCommand: /cz burn [seconds] | off.')
        end
        ImGui.SameLine()
    end
