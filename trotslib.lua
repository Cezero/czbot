...
function trotslib.StartUp(...)
   ...
   if not(mq.TLO.NetBots.Enable() and mq.TLO.NetBots.Listen() and mq.TLO.NetBots.Output() and mq.TLO.NetBots(mq.TLO.Me.Name()).MaxEndurance()) then
      mq.cmd('/multiline ; /dgt Trotsbot Startup: \agNetbots\ag is not fully enabled, enabling it ; /netbots on grab=on send=on ext=on')
   end
   --load config file
   runconfig = {}
   runconfig['ScriptList'] = {}
   IniFile = 'TB_'..mq.TLO.Me.Name()
   keyOrder = { 'settings', 'pull', 'melee', 'heal', 'buff' , 'debuff', 'cure', 'event', 'ah' , 'ab', 'ad', 'ac', 'ae', 'script'} -- Add the keys in the order you want them to be saved
   ...
   if comkeytable['excludelist'] and comkeytable.excludelist[mq.TLO.Zone.ShortName()] then runconfig['ExcludeList'] = comkeytable.excludelist[mq.TLO.Zone.ShortName()]
   else runconfig['ExcludeList'] = '' end
   if comkeytable['prioritylist'] and comkeytable.prioritylist[mq.TLO.Zone.ShortName()] then runconfig['PriorityList'] = comkeytable.prioritylist[mq.TLO.Zone.ShortName()]
   else runconfig['PriorityList'] = '' end
...
function trotslib.EquipGear(geartoequip)
   ...
   if canuse and wornslot then 
      mq.cmdf('/dgt \ayTrotsbot:\ax\ayattempting to replace \ar%s\ax with \ag%s\ax in slot %s', curitemlink, toequipitemlink, slotname)
   else
            ...
            if mq.TLO.InvSlot(slot).Item.AugSlot(i)() then
               ...
               if aug then
                  ...
                  if mq.TLO.Window("ConfirmationDialogBox") then
                     mq.cmd('/yes')
                     mq.delay(3000, function() if (not mq.TLO.InvSlot(slot).Item.AugSlot(augslot)()) and mq.TLO.Cursor.ID() then return true end end)
                     mq.delay(100)
                     mq.cmd('/autoinv')
                  end
                  mq.delay(2000, function() if mq.TLO.InvSlot(slot).Item.AugSlot(i)() then return true end end)
                  if not mq.TLO.InvSlot(slot).Item.AugSlot(i)() then
                     mq.cmdf("/dgt \ayTrotsbot:\axI've removed \ag%s\ax in \ag%s\ax succesfully", RemovedAugs[i], curitem)
                  else
                     mq.cmdf("/dgt \ayTrotsbot:\ax\arSomething went wrong removing %s from %s most likely need distillers", RemovedAugs[i], curitem)
                  end
               end
            end
         end
...
function trotslib.CharState(...)
   ...
   if runconfig['campstatus'] and trotslib.calcDist2D(mq.TLO.Me.X(), mq.TLO.Me.Y(), runconfig['makecampx'], runconfig['makecampy']) and trotslib.calcDist2D(mq.TLO.Me.X(), mq.TLO.Me.Y(), runconfig['makecampx'], runconfig['makecampy']) > myconfig.settings['acleash'] then trotsmove.MakeCamp('return') end
...
function trotslib.LoadConfig(...)
   ...
   if not myconfig then print('making new config') myconfig = {} end
   ...
   if string.find(mq.TLO.Ini(IniFile)(), 'Settings') then
      ...
      if myconfig.buff.count and myconfig.buff.count > 0 then
         for i=1, myconfig.buff.count do
            ...
            trotslib.LoadINIVar('AB'..i, 'TarType', 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz', 'class', IniFile)
      ...
      if myconfig.heal.count and myconfig.heal.count > 0 then
         for i=1, myconfig.heal.count do
            ...
            trotslib.LoadINIVar('AH'..i, 'Class', 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , 'class', IniFile)
...
function trotslib.ImportCom()
   if string.find(mq.TLO.Ini('TBCommon')(), 'Settings') then
      ...
      for prvalue in string.gmatch(tostring(prioritylistimp), "([^|]+)") do
         if (prvalue) then
            comkeytable.prioritylist[tostring(prvalue)] = mq.TLO.Ini('TBCommon', 'PriorityList', string.lower(prvalue))()
...
function trotslib.LoadINIVar(IniSection,IniVar,IniValue,MacroVar,MyIni)
   ...
   if (IniString =="NOTFOUND") then
      ...
      if not myconfig[string.lower(IniSection)] then myconfig[string.lower(IniSection)] = {} end 
      if tonumber(IniString) then myconfig[string.lower(IniSection)][MacroVar] = tonumber(IniString)
...
function ProcessImmuneList(immuneID)
   ...
   if immuneID and spell and mq.TLO.Spawn(immuneID).ID() and mq.TLO.Spawn(immuneID).Type() ~= 'Corpse' then
      ...
      if not ImmuneList[spell][zone][mq.TLO.Spawn(immuneID).CleanName()] then  
         ...
         mq.cmdf('/dgt \ayTrotsbot:\ax%s is \\arIMMUNE\\ax to spell \\ag%s\\ax, adding to the ImmuneList', mq.TLO.Spawn(immuneID).CleanName(), spell)
         mq.pickle('tbimmune.lua', ImmuneList)
      end
   end
...
function trotslib.DistanceCheck(Sub, ID, EvalID)
   ...
   if myconfig[Sub..ID] then spell = myconfig[Sub..ID].spell
   else spell = Sub end
...
function trotslib.LoadSpell(Sub, ID)
   ...
   if gem == 'item' then
      if mq.TLO.Me.ItemReady(spell)() then return true 
      else 
         return false end
   end
   if gem == 'disc' then
...
function trotslib.InterruptCheck()
   ...
   local spellname = nil
   if sub and spell then spellname = myconfig[sub..spell].spell or (myconfig[sub..spell].gem == 'item' and mq.TLO.FindItem(myconfig[sub..spell].spell).Spell()) end
   ...
   local spellid = mq.TLO.Spell(myconfig[sub..spell].spell).ID() or (myconfig[sub..spell].gem == 'item' and mq.TLO.FindItem(myconfig[sub..spell].spell).Spell.ID())
   local spelldur = mq.TLO.Spell(spellname).MyDuration.TotalSeconds() or (myconfig[sub..spell].gem == 'item' and mq.TLO.FindItem(myconfig[sub..spell].spell).Spell.MyDuration())
   ...
   if criteria ~= 'corpse' then
      if mq.TLO.Target.Type() == 'Corpse' and criteria ~='corpse' then mq.cmd('/multiline ; /interrupt ; /squelch /target clear ; /echo My target is dead, interrupting') end
   end
   ...
   if sub == 'ah' and criteria ~= 'corpse' then
      if mq.TLO.Target.PctHPs() and AHThreshold and AHThreshold[spell] and AHThreshold[spell][criteria] and (AHThreshold[spell][criteria] + (math.abs(AHThreshold[spell][criteria] - 100) * myconfig.heal['interruptlevel'])) <= mq.TLO.Target.PctHPs() and mq.TLO.Target.ID() == target then
         mq.cmdf('/multiline ; /interrupt ; /echo Interrupting Spell %s, target is above the threshold', myconfig['ah'..spell]['spell'])
         while(mq.TLO.Me.CastTimeLeft() > 0) do
            mq.cmd('/interrupt')
            mq.delay(100)
         end
   ...
   if mq.TLO.Me.CastTimeLeft() > 0 and (sub == 'ad' or sub == 'ab') and spelldur and spelldur > 0 and mq.TLO.Me.Class.ShortName() ~= 'BRD' then
      ...
      if mq.TLO.Target.ID() == target and mq.TLO.Target.BuffsPopulated() and buffid and buffstaleness < 2000 and buffdur>(spelldur * .10) then
         if sub == 'ab' then
            if mq.TLO.Spell(spellid).StacksTarget() then 
               mq.cmdf('/multiline ; /echo Interrupt %s, buff does not stack on target: %s ; /interrupt', spellname, spellname, targetname)
            end
      ...
      if mq.TLO.Target.ID() == target and mq.TLO.Target.BuffsPopulated() and mq.TLO.Spell(spellid).StacksTarget() == 'FALSE' then
         if sub == 'ab' then
            mq.cmdf('/multiline ; /dgt \ayTrotsbot:\axInterrupt %s, buff does not stack on target: %s ; /interrupt', spellname, spellname, targetname)
            while(mq.TLO.Me.CastTimeLeft() > 0) do
               mq.cmd('/interrupt')
               mq.delay(100)
            end
            ...
            CurSpell = {}
         elseif sub == 'ad'  then 
            mq.cmdf('/multiline ; /dgt \ayTrotsbot:\axInterrupt %s on MobID %s Name %s, debuff does not stack ; /interrupt', spellname, target, targetname)
   ...
   if mq.TLO.Me.CastTimeLeft() > 0 and sub == 'ac' and false then
      ...
      if tarname then 
         ...
         if curtar and nbid and curtar == target and buffspop and not nbdebuff then 
            mq.cmdf('/multiline ; /dgt \ayTrotsbot:\axInterrupt %s, is no longer %s ; /interrupt', spellname, criteria)
            while(mq.TLO.Me.CastTimeLeft() > 0) do
               mq.cmd('/interrupt')
               mq.delay(100)
            end
...
function trotslib.CastSpell(index, EvalID, classhit, sub)
   ...
   if mq.TLO.Me.Class.ShortName() == 'BRD' then 
      if (myconfig.settings['domelee'] and runconfig['MobCount']>0 and classhit~='notanktar' and not mq.TLO.Me.Combat()) then trotsmelee.AdvCombat() end
      if type(gem) == 'number' then
   ...
   if type(gem) == 'number' or gem == 'item' or gem == 'alt' or 'script' then
      if EvalID == 1 and (classhit == 'self' or classhit == 'grp') then 
         ...
         elseif gem == 'script' then trotslib.RunScript(spell, sub, index) 
         end
      else 
      ...
      while mq.TLO.Me.CastTimeLeft() > 0 and mq.TLO.Me.CastTimeLeft() < 10000 do
         ...
         if mq.TLO.Target.ID() and (mq.TLO.Target.ID() == 0 or string.find(mq.TLO.Target.Name(), 'corpse')) then
            ...
            if mq.TLO.NetBots(myconfig.settings['TankName']).TargetID() and mq.TLO.NetBots(myconfig.settings['TankName']).TargetID() ~= 0 and myconfig.settings['domelee'] then
               if tarname and string.find(mq.TLO.Target.Name(), 'corpse') then mq.cmd('/squelch /multiline ; /attack off ; /stick off ; /target clear') end
               --if runconfig.domelee then trotsmelee.AdvCombat() end
            end
         end
      end
      mq.delay(500, function() if mq.TLO.Me.Song(spell).Duration() and mq.TLO.Me.Song(spell).Duration()> 10000 then return true end end)
   end
   --print('castwait5')
   mq.doevents()
      ...
      if (mq.TLO.Spell(spell).MyDuration() and tonumber(mq.TLO.Spell(spell).MyDuration()) > 0) then
         ...
         if mq.TLO.Target.Buff(spell).ID() or mq.TLO.Me.Class.ShortName() == 'BRD' and not MissedNote then
            ...
            if not CurSpell.resisted then
                  if debug then print('saving ', EvalID, ' and spell ', spell, ' with duration ', myduration, ' time is ', mq.gettime()) end
      ...
      for i=1,cnt do
         ...
         if entry and type(entry.alias) == 'string' and entry.alias ~= '' then
            ...
            if entry.gem == 'disc' then
               ...
               known = entry.spell and entry.spell ~= 0 and entry.spell ~= '' and mq.TLO.Me.CombatAbility(entry.spell)() ~= nil
            else
...
function trotslib.GroupInvite(groupldr, groupmembers, grptype)
   ...
   for raidmember, _ in pairs(groupmembers) do
      ...
      elseif grptype == 'raid' then
         mq.cmdf("/dgt \ayTrotsbot:\ax\ar%s's\ax group member \ar%s\ax is not in the zone, skipping", groupldr, raidmember )
      end
   end
end
...
function Event_CHChain(line, arg1)
   ...
   if not trotslib.DistanceCheck('complete heal', 0, tankid) then mq.cmdf('/rs Tank %s is out of range of complete heal!', chchaintank) end
...
function Event_FTELocked()
   ...
   if spawn.ID() and spawn.ID()>0 then mq.cmdf('/dgt \ayTrotsbot:\ax\arUh Oh, \ag%s\ax is \arFTE locked\ax to someone else!', spawn.Name()) end
   ...
   if myconfig.settings['dopull'] then print('clearing pull target ') APTarget = false end
...
function cmdparse(...)
   ...
   if togglelist[args[1]] then
      ...
      elseif args[2] == 'off' then
         ...
         else myconfig.settings[args[1]] = false  myconfig.settings[args[1]] = true end
      end
      if myconfig.settings['domelee'] then trotsmelee.LoadMeleeConfig() end
      if myconfig.settings['dopull'] then trotspull.LoadPullConfig() end
      if myconfig.settings['doheal'] then trotsheal.LoadHealConfig() end
   ...
   if args[1] == 'makecamp' then
      ...
      elseif not args[2] then
         if runconfig['campstatus'] then trotsmove.MakeCamp('off')
         else trotsmove.MakeCamp('on') end
      end
   ...
   if args[1] == 'abort' then
      if not args[2] then
         ...
         if runconfig['acmatarget'] then runconfig['acmatarget'] = nil end
         if myconfig.settings['domelee'] then myconfig.settings['domelee'] = false meleeabort = true end
         if myconfig.settings['dodebuff'] then myconfig.settings['dodebuff'] = false debuffabort = true end
   ...
   if args[1] == 'leash' then
      ...
      else mq.cmd('/dgt \ayTrotsbot:\axNo camp set, cannot leash')
      end
   end
   ...
   if args[1] == 'attack' then
      ...
      if (KillTarget) then mq.cmdf('/dgt \ayTrotsbot:\ax\arEngaging\ax \ay%s\ax now', mq.TLO.Spawn(KillTarget).CleanName())
      else mq.cmd('/dgt \ayTrotsbot:\ax\ar Tank has no target, cannot engage') end
   end
   ...
   if args[1] == 'offtank' then
      ...
      elseif 
         string.lower(args[2]) == 'true' or string.lower(args[2]) == 'on' then myconfig.melee.offtank = true
      elseif 
         string.lower(args[2]) == 'false' or string.lower(args[2]) == 'off' then myconfig.melee.offtank = false
      else 
         mq.cmdf('/dgt \ayTrotsbot:\ax%s is an invalid value for offtank, please use true, on, false, off, or leave it blank to toggle', args[2])
         return false
      end
            ...
            for i=1, myconfig.debuff['count'] do
               for value in tostring(myconfig['ad'..i]['alias']):gmatch("[^|]+") do
                  if value == args[2] and (args[3] ~= 'on' and args[3] ~= 'off') then 
                     mq.cmdf('/dgt \ayTrotsbot:\ax\agCasting\ax %s on %s', myconfig['ad'..i].spell, mq.TLO.Spawn(target).CleanName())
                  ...
                  elseif args[3] and value == args[2] then
                     if args[3] == 'on' then 
                        myconfig['ad'..i].tarcnt = 1 mq.cmdf('/dgt \ayTrotsbot:\axEnabling \ag%s\ax', myconfig['ad'..i].spell) 
                        if not myconfig.settings.dodebuff then 
                           trotsdebuff.LoadDebuffConfig()
                           myconfig.settings.dodebuff = true 
                        end
                     end
                     if args[3] == 'off' then myconfig['ad'..i].tarcnt = 0 mq.cmdf('/dgt \ayTrotsbot:\axDisabling \ag%s\ax', myconfig['ad'..i].spell) end
                  end
               end
            end
         end
         if myconfig.buff['count'] and myconfig.buff['count'] > 0 then
            for i=1, myconfig.buff['count'] do
               for value in tostring(myconfig['ab'..i]['alias']):gmatch("[^|]+") do
                  if value == args[2] and (args[3] ~= 'on' and args[3] ~= 'off') then 
                     mq.cmdf('/dgt \ayTrotsbot:\ax\agCasting\ax %s on %s', myconfig['ab'..i].spell, mq.TLO.Spawn(target).CleanName())
                     if not trotslib.LoadSpell('ab', i) then 
                        mq.delay(3000)
                        if not trotslib.LoadSpell('ab', i) then mq.cmdf('/dgt \ayTrotsbot:\ax\arCast command spell %s not ready!', myconfig['ab'..i].spell)
                        else 
                  ...
                  elseif args[3] and value == args[2] then
                     if args[3] == 'on' then 
                        myconfig['ab'..i].tarcnt = 1 mq.cmdf('/dgt \ayTrotsbot:\axEnabling \ag%s\ax', myconfig['ab'..i].spell) 
                        if not myconfig.settings.dobuff then 
                           trotsbuff.LoadBuffConfig()
                           myconfig.settings.dobuff = true 
                        end
                     end
                     if args[3] == 'off' then myconfig['ab'..i].tarcnt = 0 mq.cmdf('/dgt \ayTrotsbot:\axDisabling \ag%s\ax', myconfig['ab'..i].spell) end
                  end
               end
            end
         end
         if myconfig.heal['count'] and myconfig.heal['count'] > 0 then
            for i=1, myconfig.heal['count'] do
               for value in tostring(myconfig['ah'..i]['alias']):gmatch("[^|]+") do
                  if value == args[2] and (args[3] ~= 'on' and args[3] ~= 'off') then 
                     mq.cmdf('/dgt \ayTrotsbot:\ax\agCasting\ax %s on %s', myconfig['ah'..i].spell, mq.TLO.Spawn(target).CleanName()) 
                     if not trotslib.LoadSpell('ah', i) then 
                        mq.delay(3000)
                        if not trotslib.LoadSpell('ah', i) then mq.cmdf('/dgt \ayTrotsbot:\ax\arCast command spell %s not ready!', myconfig['ah'..i].spell)
                        else 
                  ...
                  elseif args[3] and value == args[2] then
                     if args[3] == 'on' then 
                        myconfig['ah'..i].tarcnt = 1 mq.cmdf('/dgt \ayTrotsbot:\axEnabling \ag%s\ax', myconfig['ah'..i].spell) 
                        if not myconfig.settings.doheal then 
                           trotsheal.LoadHealConfig()
                           myconfig.settings.doheal = true 
                        end
                     end
                     if args[3] == 'off' then myconfig['ah'..i].tarcnt = 0 mq.cmdf('/dgt \ayTrotsbot:\axDisabling \ag%s\ax', myconfig['ah'..i].spell) end
                  end
               end
            end
         end
         if myconfig.cure['count'] and myconfig.cure['count'] > 0 then
            for i=1, myconfig.cure['count'] do
               for value in tostring(myconfig['ac'..i]['alias']):gmatch("[^|]+") do
                  if value == args[2] and (args[3] ~= 'on' and args[3] ~= 'off') then 
                     mq.cmdf('/dgt \ayTrotsbot:\ax\agCasting\ax %s on %s', myconfig['ac'..i].spell, mq.TLO.Spawn(target).CleanName())                     
                     if not trotslib.LoadSpell('ac', i) then 
                        mq.delay(3000)
                        if not trotslib.LoadSpell('ac', i) then mq.cmdf('/dgt \ayTrotsbot:\ax\arCast command spell %s not ready!', myconfig['ac'..i].spell)
                        else 
                  ...
                  elseif args[3] and value == args[2] then
                     if args[3] == 'on' then 
                        myconfig['ac'..i].tarcnt = 1 mq.cmdf('/dgt \ayTrotsbot:\axEnabling \ag%s\ax', myconfig['ac'..i].spell)
                        if not myconfig.settings.docure then 
                           trotscure.LoadCureConfig()
                           myconfig.settings.docure = true 
                        end
                     end
                     if args[3] == 'off' then myconfig['ac'..i].tarcnt = 0 mq.cmdf('/dgt \ayTrotsbot:\axDisabling \ag%s\ax', myconfig['ac'..i].spell) end
                  end
               end
            end
         end
         if myconfig.event['count'] and myconfig.event['count'] > 0 then
            for i=1, myconfig.event['count'] do
               for value in tostring(myconfig['ae'..i]['alias']):gmatch("[^|]+") do
                  if value == args[2] and (args[3] ~= 'on' and args[3] ~= 'off') then 
                     mq.cmdf('/dgt \ayTrotsbot:\ax\agCasting\ax %s on %s', myconfig['ae'..i].spell, mq.TLO.Spawn(target).CleanName())                     
                     if not trotslib.LoadSpell('ae', i) then 
                        mq.delay(3000)
                        if not trotslib.LoadSpell('ae', i) then mq.cmdf('/dgt \ayTrotsbot:\ax\arCast command spell %s not ready!', myconfig['ae'..i].spell)
                        else 
                  ...
                  elseif args[3] and value == args[2] then
                     if args[3] == 'on' then 
                        myconfig['ae'..i].tarcnt = 1 mq.cmdf('/dgt \ayTrotsbot:\axEnabling \ag%s\ax', myconfig['ae'..i].spell) 
                        if not myconfig.settings.doevent then 
                           trotsevent.LoadEventConfig()
                           myconfig.settings.doevent = true 
                        end
                     end
                     if args[3] == 'off' then myconfig['ae'..i].tarcnt = 0 mq.cmdf('/dgt \ayTrotsbot:\axDisabling \ag%s\ax', myconfig['ae'..i].spell) end
                  end
               end
            end
         end
   ...
   if args[1] == 'addspell' then
      ...
      if not subfound then
         mq.cmdf('/dgt \ayTrotsbot:\ax%s is not a valid trotsbot sub please use heal, buff, debuff, event, or cure', sub)
         return false
      end
      if (not myconfig[sub].count and tonumber(key) > 1) or (tonumber(key) ~= 1 and myconfig[sub].count and (tonumber(key) > tonumber(myconfig[sub].count))) then
         mq.cmdf('/dgt \ayTrotsbot:\ax%s is higher than the current spell count for %s, please use a valid existing entry key (ex: ab1 key would be 1)', key, sub)
         return false
      end
      local temploadconfig = loadfile(path)
      if temploadconfig then
      ...
      if sub == 'event' then 
         trotsevent.LoadEventConfig() 
         tempconfig['ae'..count] = {gem = 0, spell = 0, minmana = 0, maxmana = 100, minhp = 0, maxhp = 100, minendur = 0, maxendur = 100, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , precondition = true}
      end
      if sub == 'heal' then
         trotsheal.LoadHealConfig() 
         tempconfig['ah'..count] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'pc pet group hp50 war shd pal rng mnk rog brd bst ber shm clr dru wiz mag enc nec tnt mypet self', priority = false, precondition = true}
      end
      if sub == 'buff' then 
         trotsbuff.LoadBuffConfig() 
         tempconfig['ab'..count] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , spellicon = 0, precondition = true}
      end
      if sub == 'debuff' then 
         trotsdebuff.LoadDebuffConfig() 
      ...
      if sub == 'cure' then 
         trotscure.LoadCureConfig() 
         tempconfig['ac'..count] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, curetype = "all", tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , priority = false, precondition = true}
      end
      writeConfigToFile(tempconfig, path, keyOrder)
      tempconfig = {}
      ...
      for i = tempconfig[sub].count, key, -1 do
         ...
         elseif i == tonumber(key) then
            ...
            if sub == 'event' then 
               myconfig['ae'..i] = {gem = 0, spell = 0, minmana = 0, maxmana = 100, minhp = 0, maxhp = 100, minendur = 0, maxendur = 100, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , precondition = true}
               tempconfig['ae'..i] = {gem = 0, spell = 0, minmana = 0, maxmana = 100, minhp = 0, maxhp = 100, minendur = 0, maxendur = 100, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , precondition = true}
            end
            if sub == 'heal' then 
               tempconfig['ah'..i] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'pc pet group hp50 war shd pal rng mnk rog brd bst ber shm clr dru wiz mag enc nec tnt mypet self', priority = false, precondition = true}
               myconfig['ah'..i] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'pc pet group hp50 war shd pal rng mnk rog brd bst ber shm clr dru wiz mag enc nec tnt mypet self', priority = false, precondition = true}
            end
            if sub == 'buff' then 
               myconfig['ab'..i] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , spellicon = 0, precondition = true}
               tempconfig['ab'..i] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , spellicon = 0, precondition = true}
            end
            if sub == 'debuff' then 
            ...
            if sub == 'cure' then 
               myconfig['ac'..i] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, curetype = "all", tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , priority = false, precondition = true}
               tempconfig['ac'..i] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, curetype = "all", tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , priority = false, precondition = true}
            end
         end
      end
      writeConfigToFile(tempconfig, path, keyOrder)
      ...
      mq.cmdf('/dgt \ayTrotsbot:\axadded new %s entry at a%s%s, all existing entries moved down 1 position', sub, subltr, key)
   end
   ...
   if args[1] == 'echo' then
      ...
      if myconfig[sub][key] then mq.cmdf('/dgt \ayTrotsbot:\ax\ag%s\ax is set as \ay%s\ay', args[2], myconfig[sub][key])
      else mq.cmdf('/dgt \ayTrotsbot:\ax\ar%s\ar is not a valid trotsbot value', args[2]) end
   end
   ...
   if args[1]== 'chchain' then
      ...
      if args[2] == 'setup' then
         ...
         if mq.TLO.Me.Gem(spell)() ~= gem then
            if mq.TLO.Me.Book(spell)() then 
               ...
               mq.cmdf('/dgt \ayTrotsbot:\axTrotsbot CHChain: Spell %s not found in your book, failed to start CHChain', spell)
               return false
            end
         end
         chchainlist = args[3]
         ...
         for v in string.gmatch(chchainlist, "([^,]+)") do
            ...
            if chnextclr then chnextclr = v break end
            if string.lower(v) == string.lower(mq.TLO.Me.CleanName()) then 
               dochchain = true
               chnextclr = true
            end
      ...
      if args[2] == 'tank' then
         if mq.TLO.Spawn('='..args[3]) then chchaintank = args[3] chtanklist = {} end
         ...
         if raidmembers > 0 and args[3] then
            ...
            for i=1, raidmembers do
               ...
               if groupldr and raidmember and groupnum then 
                  ...
                  comkeytable.raidlist[raidname].leaders[groupnum] = raidmember
                  if debug then printf('saving leader of group %s as %s', groupnum, comkeytable.raidlist[raidname].leaders[groupnum]) end
               elseif raidmember and groupnum then
                  ...
                  if debug then printf('saving member of group %s as %s', groupnum, comkeytable.raidlist[raidname].groups[groupnum][raidmember]) end
               end
            end
         mq.pickle('TBCommon.lua', comkeytable)
