...
| **chchain**       | **on** — enable on this cleric (`ch_healers`). **off** / **stop** — disable and restore settings. **start** — kickoff chain. **test** — single cast. **delay** `[ms]` — baton delay. | Complete Heal chain via czactor. See [CHChain configuration](chchain-configuration.md). |
