...
- **Runtime only (not in config file):** **ExcludeList**, **PriorityList**, **CharmList** (pull exclude/priority and charm targets), **pullarc** (directional pull), bard **dosongs** (twist on/off via **/cz togglesongs** or Status **Songs**), and **no-combat zone Enabled checkboxes** (temporary disable per zone for the session) are set at runtime via **/cz exclude**, **/cz priority**, **/cz charm** (add/remove), **/cz xarc**, **/cz togglesongs**, or the GUI Mob lists tab. Per-zone mob lists are stored in **cz_common.lua** under **zones**[*zoneShortName*] (**excludelist**, **prioritylist**, **charmlist**, **nukeFlavors**, **nukeFlavorsAutoDisabled**, **immune**). The global **noCombatZones** array is also in **cz_common.lua** (top-level, not under zones). Changes to lists and no-combat zones are saved automatically when you add or remove entries in the GUI or via commands.
