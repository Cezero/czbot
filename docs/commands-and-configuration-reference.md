...
| Command          | Arguments               | Purpose                                                                                                           |
| ---------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **attack**       | optional `<name>`       | Engage the Main Assist’s target **immediately** (ignores the “assist at” percentage) and **broadcast `attack` on czactor** so all in-scope group/raid peers engage the same spawn ID. If `<name>` is given, resolve that player’s target and broadcast it. Rejected if the target is a protected NPC (soulbinder/translocator). Blocked when near primary bind point. The engagement **persists** until the target dies or an override occurs: **`/cz abort`**, turning off **domelee**, or issuing another **`/cz attack`**. Normal assist-at logic resumes after the engagement ends. |
