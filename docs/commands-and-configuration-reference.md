...
| **camphere** | `group`, `raid`, `off`, or `stop` (auto-detect group/raid when omitted) | Leader command: camp locally (skipped in hunter/roam pull), then broadcast **`camp_here`** on the czbot Actor channel so peers camp at their own position. Remembers scope; `off`/`stop` broadcasts **`camp_here_off`**. Mutually exclusive with followme. |
| **follow**   | `<name>`, `me`, or omit  | Follow the named character (disables make camp). With `me` or no name, follow TankName. Peers can also receive follow via Actor **`follow_me`** from `/cz followme`. |
...
| **followme** | `group`, `raid`, `off`, or `stop` (auto-detect group/raid when omitted) | Leader command: clear local camp/follow, then broadcast **`follow_me`** on the czbot Actor channel. Remembers scope; `off`/`stop` broadcasts **`follow_me_off`**. Mutually exclusive with camphere. |
...
| **reloadcommon**    | —                      | Reload `cz_common.lua` from disk (**read-only**; never writes) and refresh current-zone exclude/priority/charm lists, **ma_list** / **mt_list** / **ot_list**, and nuke flavor auto-disable state. Does not reset temporary no-combat zone enable/disable checkboxes (session-only). Manual fallback when czactor sync is unavailable. If the main file is unreadable, uses **cz_common.lua.bak** in memory only (does not rewrite main). Read failures retry each tick; after 5 consecutive failures the bot pauses (`/czp off` to resume after fixing the file). Explicit edits (commands/GUI) are the only runtime writes; they use **cz_common.lock** and merge with disk before save. Startup creates the file once if missing. |

**Multi-box:** Persisted edits to `cz_common.lua` auto-propagate to peers via the czactor **`common_sync`** message (same machine: memory reload only; remote machines: disk apply under lock). Use **`/cz reloadcommon`** as a manual fallback. Zone changes reload common read-only only. List edits save under **cz_common.lock** with merge-before-write. See [CZBot Actor channel](czbot-actor-channel.md#cz_common-sync-common_sync) and [Automatic MA/MT Selection](automatic-ma-mt-selection.md#editing-lists-and-multi-box-sync).
...
| Command          | Arguments               | Purpose                                                                                                           |
| ---------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------- |
| **attack**       | optional `<name>`       | Engage the Main Assist’s target **immediately** (ignores the “assist at” percentage) and **broadcast `attack` on czactor** so all in-scope group/raid peers **silently** lock the same spawn ID. Issuer announces once; peers do not log engage. Prefer one-box (do not `/dgga`). If `<name>` is given, resolve that player’s target and broadcast it. Rejected if the target is a protected NPC (soulbinder/translocator). Blocked when near primary bind point. The engagement **persists** until the target dies or an override occurs: **`/cz abort`**, turning off **domelee**, or issuing another **`/cz attack`**. Normal assist-at logic resumes after the engagement ends. |
| **disengage**    | —                       | Release the current combat target (stick off, attack off). On the **Main Assist**, also broadcasts **`ma_disengage`** so peers resume follow-leash behavior. On other bots, local disengage only. Does not disable domelee/dodebuff (unlike **`abort`**). |
...
| **actor**        | `ping` / `status` | CZBot Actor channel diagnostics (peers, queue depth/drain/drop, MA/MT overrides). Mailbox registers at startup; restart macro if `mailbox=MISSING`. See [CZBot Actor channel](czbot-actor-channel.md). |
...
| **chchain**       | **on** — local participate flag (like doHeal; persists). **off** — local opt-out. **start** — from any bot: arm slot clocks on enabled clerics. **stop** — deactivate chain (keep participate flag). **test** — single cast. **delay** `[ms]` — slot delay. | Complete Heal chain via absolute slot timing. See [CHChain configuration](chchain-configuration.md). |
...
StoredConfig = {
  ...
  pull = { ... },
  melee = { ... },
  heal = { interruptlevel = 0.80, xttargets = 0, spells = { ... } },
  buff = { spells = { ... } },
  debuff = { spells = { ... } },
  cure = { spells = { ... } },
  script = {}
