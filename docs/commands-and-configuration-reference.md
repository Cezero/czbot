...
| **chchain**       | **on** — local participate flag (like doHeal; persists). **off** — local opt-out. **start** — activate + kickoff. **stop** — deactivate chain (keep participate flag). **test** — single cast. **delay** `[ms]` — baton delay. | Complete Heal chain via czactor. See [CHChain configuration](chchain-configuration.md). |
