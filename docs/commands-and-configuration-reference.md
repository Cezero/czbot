...
| **chchain**       | **on** — local participate flag (like doHeal; persists). **off** — local opt-out. **start** — from any bot: kickoff first `ch_healers` entry. **stop** — deactivate chain (keep participate flag). **test** — single cast. **delay** `[ms]` — baton delay. | Complete Heal chain via czactor. See [CHChain configuration](chchain-configuration.md). |
