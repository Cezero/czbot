...
| **actor**        | `ping` / `status` | CZBot Actor channel diagnostics (peers, OT claims, MA/MT overrides). Mailbox registers at startup; restart macro if `mailbox=MISSING`. See [CZBot Actor channel](czbot-actor-channel.md). |
