...
| **reloadcommon**    | —                      | Reload `cz_common.lua` from disk (read-only; does not write) and refresh current-zone exclude/priority/charm lists, **ma_list** / **mt_list**, and nuke flavor auto-disable state. Does not reset temporary no-combat zone enable/disable checkboxes (session-only). Use on each bot after another client edits shared lists so runtime state matches disk. If the main file is corrupt or missing, loads from **cz_common.lua.bak** when available. Edits use a lock file and merge with disk before save. |

**Multi-box:** All bots sharing `cz_common.lua` should run **`/cz reloadcommon`** after one bot edits exclude/priority/charm lists or **ma_list** / **mt_list** in the GUI, so each client's runtime lists match disk. Zone changes reload common read-only (no auto-save). List edits reload from disk, union-merge with existing entries, then save under **cz_common.lock**. See [Automatic MA/MT Selection](automatic-ma-mt-selection.md#editing-lists-and-multi-box-sync).
