...
| **camphere** | `group`, `raid`, `off`, or `stop` (auto-detect group/raid when omitted) | Leader command: camp locally (skipped in hunter/roam pull), then `/rc group\|raid /cz makecamp on` so each bot camps at its own position. Remembers scope; `off`/`stop` sends `/rc group\|raid /cz stop` and clears local camp. Mutually exclusive with followme. |
...
| **followme** | `group`, `raid`, `off`, or `stop` (auto-detect group/raid when omitted) | Leader command: clear local camp/follow, then `/rc group\|raid /cz follow <this toon>`. Remembers scope; `off`/`stop` sends `/rc group\|raid /cz stop`. Switching scope auto-stops the previous scope. Mutually exclusive with camphere. |
