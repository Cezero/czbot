...
- **tickAggroing:** Sub-phases: aggro_wait_target (deadline or target set), aggro_wait_cast (cast done → aggro_wait_ranged), aggro_wait_stop_moving (deadline or not moving), aggro_wait_ranged (1.5s pause after ranged/disc/cast pull; early exit on XTarget or ToT aggro, else melee fallback). Transition to returning when pull target has aggro (XTarget or ToT) and min wait (1.5s) has passed. Else try pull ability (disc/ability/alt/gem/ranged/melee/warp); if moving and need to stand still, set aggro_wait_stop_moving.
