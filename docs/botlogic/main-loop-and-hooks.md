...
- **Busy:** If `state.isBusy()` is true and the payload has a numeric `priority`, only hooks with `hook.priority <= payload.priority` run. That keeps the owning hook (e.g. doPull at 800) and all higher-priority hooks (zoneCheck through doMelee) running; doHeal, doDebuff, doBuff, doCure, doMiscTimer are skipped until state is cleared.
...
| Name | Priority | Provider | runWhenDead | runWhenPaused | Deep-dive |
|------|----------|----------|-------------|--------------|-----------|
...
No hooks currently set `runWhenPaused = true`. Built-in hooks (zoneCheck, doEvents, charState, doMiscTimer) are implemented in `botlogic.lua` and registered via `registerHookFn` before `registerAllFromConfig()`; providers supply their hook function via `getHookFn(name)`.
