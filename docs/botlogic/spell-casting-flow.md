...
CastSpell sets `CurSpell`, may wait for movement or target, then issues the cast and sets `runState = 'casting'`. Spell/item/alt casts route through `lib/casting.lua` and skip the precast target step when possible; completion and resist/immune are detected via normalized `casting.status()/result()/storedSpellId()` in re-entry.
...
flowchart TB
    ...
    Moving -->|No| CastingLib{spell/item/alt cast type?}
    CastingLib -->|Yes| DoCasting["casting.start(request); viaCastingLib, spellid"]
    CastingLib -->|No| TargetOk{Target.ID == EvalID?}
...
- **SpellCheck:** Reagents, mana/endurance, gem ready, etc. For gem spells, verifies the spell is in the character's book (`mq.TLO.Me.Book(spell)`); if not, the entry is disabled and the cast is aborted.
...
- **precast:** If target is not set (and not using the casting library path), we /tar and set a 1s deadline; next tick we re-call CastSpell. When the caster is the main tank, in combat, and the cast target is self, the precast target step is skipped so the MT does not untarget the mob.
- **casting library:** For gem/item/alt, `casting.start(...)` runs target-aware dispatch, memorization if needed, and retries for configured cast types. `CurSpell.viaCastingLib` and `CurSpell.spellid` are set before entering `casting` phase. Completion is detected via `casting.status()/result()/storedSpellId()`. **Memorization** (`status` contains `M`) is treated as part of the busy cast state: no stuck clear, no interrupt checks, and no sit/stand churn until memorization finishes.
...
flowchart TB
    ...
    P1 -->|No| P1b{phase == casting and viaCastingLib?}
    P1b -->|Yes| CastLibComplete{status not C and not M, storedSpellId == spellid?}
    CastLibComplete -->|Yes| OnCastLib[Set resisted from result, immune.processList target if CAST_IMMUNE, OnCastComplete, afterCast, clear]
...
- **casting (via casting library):** Completion is when `casting.status()` does not contain 'C' and does not contain 'M', and `casting.storedSpellId()` equals `CurSpell.spellid`. Then set `CurSpell.resisted` from `casting.result()`; on CAST_IMMUNE call `immune.processList(CurSpell.target)` (the target spawn ID) to update the immune list for that mob. Then OnCastComplete, afterCast, clearCastingStateOrResume. No CastTimeLeft or cast_complete_pending_resist for this path.
...
Runs when `CurSpell.phase == 'casting'` for both casting-library and legacy paths (for legacy, the wait vs complete decision uses CastTimeLeft; InterruptCheck is still invoked each tick while casting). For casting-library casts, completion is determined by normalized status/result only; CastTimeLeft is not used for completion. When the backend is **memorizing** (`status` contains `M` and no cast bar), InterruptCheck is not run so memorization is never interrupted. Otherwise InterruptCheck runs and can interrupt the cast and call clearCastingStateOrResume in cases such as:
