...
- **Dead:** If DEAD or HOVER, disable pull (`botpull.DisablePull('death')`), stop follow/travel (`follow.StopFollow('death')`), clear makecamp and stored anchor (`botmove.ClearCamp('death')`), set runState 'dead', set HoverEchoTimer if unset, and if HoverTimer passed call Event_Slain. If we were dead and are now alive, clearRunState.
