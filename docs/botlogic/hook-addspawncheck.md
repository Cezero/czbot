...
- **buildCampMobList:** Uses **MobList anchor** from `getMobListAnchor` (see below). `mq.getFilteredSpawns` with `filterSpawnForCamp` (in area, not soulbinder/translocator, not in ExcludeList, not FTE-locked, TargetFilter: `0` = NPC/pet with LoS, or at level 20+ on XTarget Auto-Hater without LoS; `2` = all NPC/non-PC pets in bubble). Spawns with missing or empty `Type()` (stale TLO during despawn) are skipped silently. Sorted by ID.
