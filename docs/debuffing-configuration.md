...
- **Mob list:** The bot builds a list of valid mobs (within **acleash** and **zradius** of camp, filtered by **TargetFilter**: `0` = LoS NPCs with a level-20+ Auto-Hater LoS waiver; `2` = All NPCs). Debuffs are only cast on mobs in this list. **Soulbinders** and **translocators** are never debuffed. Debuffs are skipped in **no-combat zones** and near the **primary bind point** (bind stealth). See [Safety and stealth](safety-and-stealth.md).
...
- **notmatar + domelee:** When **`settings.domelee`** is on, notmatar casts turn attack off, suspend melee engage while the cast/song is in progress, then on completion resume the MA/assist kill target via normal assistpct/MobList gates (no camp freepick). Same path for all classes; bards use twist-once for songs but share the resume helper. See [doMelee](botlogic/hook-domelee.md) and [Bard configuration](bard-configuration.md).
...
- **stopWhen:** Skip when the target already has the listed category (e.g. **Slowed** after a resist-setup debuff like Occlusion of Sound has done its job). For **bard matar**, the gem cycles in the combat twist until the condition is met, then is dropped from the twist list. Unconditional matar debuffs (e.g. `max` equal to **assistpct**) stay in the combat twist permanently.
