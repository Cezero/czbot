...
| **dontStack** | Optional. List of debuff **categories** (from MQ Target TLO). If the target already has any of these categories (e.g. from another character), the bot will not cast this spell on that target, and will interrupt the cast if that category appears on the target while casting. For **bard matar** debuffs in the combat twist, the gem is also omitted when dontStack applies. Allowed values (see code block below). Example: for a bard snare, set `dontStack = { 'Snared' }` so the bot does not overwrite a ranger's or druid's snare. |
| **stopWhen** | Optional. List of debuff **categories**. Skip cast (and omit from **bard matar** combat twist) when the target already has any listed category — use when the spell's job is done (not "don't overwrite"). Example: `stopWhen = { 'Slowed' }` on **Occlusion of Sound** (resist debuff) so the bard stops singing it once any slow is on the MA target. **Slowed** is only valid in **stopWhen**, not **dontStack** (casters may still overwrite weaker slows). |
