...
function trotsheal.LoadHealConfig()
    ...
    for i=1, myconfig.heal['count'] do
        if i <= myconfig.heal['count'] then
            ...
            local number = 0
            local letters =''
            if not myconfig[healkey] then myconfig[healkey] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'pc pet group hp50 war shd pal rng mnk rog brd bst ber shm clr dru wiz mag enc nec tnt mypet self', priority = false, precondition = true}  end
            ...
            if myconfig[healkey]['gem'] == 'script' then
                if not myconfig['script'][myconfig[healkey]['spell']] then 
                    ...
                    myconfig['script'][myconfig[healkey]['spell']] = "test" end
...
function trotsheal.HPEval(index)
            ...
            for i=1, corpsecount do
                if AHThreshold[index]['all'] then
                    for k=1, corpsecount do
                        ...
                        if debug then print('corpse found '..nearcorpse..' netbots char is '..mq.TLO.NetBots(bots[k]).Name()) end
    ...
    if AHThreshold[index]['grp'] then
        ...
        if grpmatch >= entry.tarcnt then 
            if mq.TLO.Spell(entry.spell).TargetType() == 'Group v1' then return 1, 'grp'
            else return mq.TLO.Me.ID(), 'grp' end
        end
    end
...
function trotsheal.HealCheck()
    ...
    for i=1, myconfig.heal['count'] do
        ...
        while castspell do
            ...
            if mq.TLO.Me.Invulnerable() then print("/echo I'm DA, waiting") mq.delay(1000) end
            ...
            if trotslib.SpellCheck('ah', i) then
                if trotslib.LoadSpell('ah', i) then
                    ...
                    if EvalID then
                        -- cast the spell
                        if not trotslib.PreCondCheck('ah', i, EvalID) then castspell=false break end
