...
while not terminate do
    ...
    elseif myconfig.settings['domelee'] and runconfig['MobList'][1] and myconfig.melee['minmana'] > 0 and myconfig.settings['TankName'] ~= mq.TLO.Me.Name() then
        if (tonumber(myconfig.melee['minmana']) < mq.TLO.Me.PctMana() or mq.TLO.Me.MaxMana()==0) then trotsmelee.AdvCombat() end
    elseif myconfig.settings['TankName'] == mq.TLO.Me.Name() and runconfig['MobList'][1] then trotsmelee.AdvCombat()
    end
    ...
    if (myconfig.settings['dopull']) then
        if runconfig['MobCount'] <= myconfig.pull['chainpullcnt'] or myconfig.pull['chainpullcnt'] == 0 then
            if mq.TLO.Spawn(runconfig['acmatarget']).PctHPs() then
                local tempcnt = 0
                if myconfig.pull['chainpullcnt'] == 0 then tempcnt = myconfig.pull['chainpullcnt'] + 1 else tempcnt = myconfig.pull['chainpullcnt'] end
                if (tonumber(mq.TLO.Spawn(runconfig['acmatarget']).PctHPs())<=myconfig.pull['chainpullhp']) and runconfig['MobCount'] <= tempcnt then trotspull.AdvPull() end
            end
        end
    ...
    if myconfig.settings['dodebuff'] and myconfig.debuff['count'] and runconfig['MobList'][1] then trotsdebuff.DebuffCheck() end
