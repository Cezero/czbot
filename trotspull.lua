...
function trotspull.SetPullArc(arc)
    ...
    if not arc and runconfig['pullarc'] then mq.cmd('/echo \arTurning off Directional Pulling.') runconfig['pullarc'] = 0 
    else runconfig['pullarc'] = tonumber(arc) end
    if not runconfig['pullarc'] then return end
    -- figure left side degrees
    if (fdir - (runconfig['pullarc'] * .5)) < 0 then arcLside = 360 - ((runconfig['pullarc'] * .5) - fdir)
    else arcLside = fdir - (runconfig['pullarc'] * .5) end
    -- figure left side degrees
    if (fdir + (runconfig['pullarc'] * .5)) > 360 then arcRside = ((runconfig['pullarc'] * .5) + fdir) - 360
    else arcRside = fdir + (runconfig['pullarc'] * .5) end
    -- set current heading
    arcMid = fdir
...
function trotspull.AdvPull()
    ...
    function TagTimeCalc(trip, spawn, x, y, z)
        local tmptagtime = 0
        if trip  == 'pull' then tmptagtimer = (((mq.TLO.Navigation.PathLength('id '..spawn.ID())() + 100)/ 100) * 9000) + mq.gettime()
        elseif x and y and z and trip == 'return' then tmptagtimer = (((mq.TLO.Navigation.PathLength('locxyz '..x..','..y..','..z)() + 100)/ 100) * 18000) + mq.gettime() end
        return tmptagtimer
    end
    ...
    function trotspull.EngageCheck(spawn)
        ...
        local botdist = 0
        if bot then targetdist = trotslib.calcDist3D(mq.TLO.Spawn(targetid).X(), mq.TLO.Spawn(targetid).Y(), mq.TLO.Spawn(targetid).Z(), runconfig['makecampx'], runconfig['makecampy'], runconfig['makecampz']) end
...
-- FTE logic that both checks for FTE locked targets (added by event FTELocked) and clears the table of any mobs older than 60 seconds
    ...
    function Aggro(timer)
        ...
        if loslist[1] and not trotspull.FTECheck(loslist[1]) and not engagedmob then
            ...
            if loslist[1] then
                APTarget = loslist[1]
                mq.cmdf('/multiline ; /echo prox aggro while running %s ; /nav stop log=off ; /squelch /tar id %s', APTarget.Name(), APTarget.ID())
                mq.delay(1000, (function () if (APTarget.ID() == mq.TLO.Target.ID()) then return true end end))
                AggroMob(APTarget,timer)
                return true
            end
...
-- Aggro's aptarget and validates aggro was succesful
    function AggroMob(spawn,timer)
        ...
        while APTarget and APTarget.Type()=='NPC' and not APTarget.Aggressive() and (timer >= mq.gettime() and spawn.Type() == 'NPC') do
            ...
            if string.lower(pullab) == 'ranged' then mq.cmdf('/multiline ; /squelch /nav stop log=off ; /face fast ; /squelch attack off ; /ranged on') end
    ...
    function CampReturn(timer)
        ...
        mq.cmdf('/multiline ; /echo \ayReturning to camp ; /nav locxyz %s %s %s log=off dist=0', runconfig['makecampx'], runconfig['makecampy'], runconfig['makecampz'])
        while mq.TLO.Navigation.Active() and (timer > mq.gettime()) do
            mq.doevents()
            if MasterPause then break end
            if myconfig.pull['pullability'] == 'warp' then mq.cmdf('/warp loc %s %s %s', runconfig['makecampy'], runconfig['makecampx'], runconfig['makecampz']) end
            mq.delay(10)
            if not mq.TLO.Navigation.Paused() and APTarget and APTarget.Distance() and APTarget.Distance() > myconfig.pull['leash'] then mq.cmd('/nav pause on log=off')
            elseif mq.TLO.Navigation.Paused() and APTarget and APTarget.Distance() and APTarget.Distance() < myconfig.pull['leash'] then mq.cmd('/nav pause off log=off') end
        end
    end
    ...
    function WaitForCombat(timer)
        ...
        while not IsAPTarInCamp() and APTarget and (APTarget.Type() == 'NPC') and APTarget.Aggressive() and (timer > mq.gettime()) do
            ...
            if mq.TLO.Math.Distance(runconfig.makecampy,runconfig.makecampx,runconfig.makecampz)() > myconfig.settings.acleash then
                ...
                if myconfig.pull['pullability'] == 'warp' then 
                    mq.cmdf('/multiline ; /echo \ayReturning to camp ; /warp loc %s %s %s', runconfig['makecampy'], runconfig['makecampx'], runconfig['makecampz']) 
                end
            end
            if debug then print('wait for combat IsAPTarInCamp:',IsAPTarInCamp(), ' timer is:', timer, 'gettime is', mq.gettime()) end
            trotsdebuff.ADSpawnCheck()
    ...
    if runconfig['pulledmob'] then 
        if mq.TLO.Spawn(runconfig['pulledmob']).Distance3D()>=myconfig.settings['acleash'] then
            ...
            mq.cmdf('/echo %s id %s never made it to camp, trying to pull a mob', mq.TLO.Spawn(runconfig['pulledmob']).CleanName(), runconfig['pulledmob'])
            runconfig['pulledmob'] = nil
        end
    end
    ...
    if mq.TLO.Me.PctHPs() <= 45 then 
        APTarget = nil tagtimer = nil 
        if hptimer and hptimer <= mq.gettime() then mq.cmd('/echo HP below 45%, holding pulls') hptimer = mq.gettime() + 30000 end
        return false
    end

    if not mq.TLO.Navigation.MeshLoaded() then 
        mq.cmd('/dgt \ayTrotsbot:\axI have DoPull set TRUE but have \arno MQ2Nav Mesh loaded\ax, please generate a NavMesh before using DoPull, \arsetting DoPull to FALSE\ax')
        ...
        for iter=1, mq.TLO.Group() do
            ...
            if (grpname.Type() and string.lower(grpname.Type()) == 'corpse') then
                if deadtimer <= mq.gettime() then
                    mq.cmdf('/dgt \ayTrotsbot:\ax\ar%s\ax is in my group and \arDEAD\ax holding pulls', 'pc '..grpname.Name())
            ...
            if curmana then
                if (myconfig.pull['mana'] > curmana) and (manatimer < mq.gettime()) then
                    mq.cmdf('/dgt \ayTrotsbot:\ax%s is a healer in my group with %s %% mana which is below my HealMana threshold, holding pulls (checking mana continously, but will message again in 60 seconds)', lowmana, curmana)
    ...
    if myconfig.pull['hunter'] and runconfig['campstatus'] then print('Disabling makecamp because dopull is on with HunterMode enabled') trotsmove.MakeCamp('off') end
    ...
    while (tagtimer and tagtimer >= mq.gettime() and APTarget) do
        if debug then print('pull loop, tag timer is '..tagtimer..' mqtime is '.. mq.gettime()..' APTarget ID is '.. APTarget.ID()) end
        mq.doevents()
        mq.delay(10)
        if MasterPause then break end
        ...
        if not mq.TLO.Navigation.Active() and APTarget.Distance() and APTarget.Distance() > myconfig.pull['abilityrange'] then mq.cmdf('/nav id %s dist= 7 log=off los=on', APTarget.ID()) end
        if mq.TLO.Me.Class.ID() == 7 and mq.TLO.Me.PctHPs() < 25 then mq.cmd('/multiline ; /doability "feign death" ; /doability "mend"') APTimer = 60000 end
        ...
        if (tagtimer <= mq.gettime()) then 
            ...
            if string.lower(myconfig.pull['pullability']) == 'warp' then mq.cmdf('/warp loc %s %s %s', runconfig['makecampy'], runconfig['makecampx'], runconfig['makecampz']) end
        end
        ...
        if mq.TLO.Me.PctHPs() <= 45 then APTarget = nil tagtimer = nil end
        if APTarget then 
            if not (APTarget.ID() or (APTarget.Type() == 'CORPSE') or (APTarget.Type() == 'PET')) then APTarget = nil tagtimer = nil end
        end
        if (APTarget) then 
    ...
    local returntimer = TagTimeCalc('return', 'return', runconfig['makecampx'], runconfig['makecampy'], runconfig['makecampz'])
