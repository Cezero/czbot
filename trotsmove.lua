...
function trotsmove.FollowCall()
    ...
    if not mq.TLO.Spawn('id '..runconfig['followid']).ID() or mq.TLO.Spawn('id '..runconfig['followid']).Type() == 'Corpse' then
        if mq.TLO.Spawn('='..runconfig['followname']).ID() then runconfig['followid'] = mq.TLO.Spawn('='..runconfig['followname']).ID() end
    end
...
function trotsmove.UnStuck()
    ...
    if (mq.TLO.Navigation.PathExists('id '..runconfig['followid'])()) then
        ...
        mq.delay(5000)
        if mq.TLO.Spawn(runconfig['followid']).Distance3D() and stuckdistance>=mq.TLO.Spawn(runconfig['followid']).Distance3D()+10 then stucktimer = mq.gettime() + 60000 return end
    end
    ...
    if mq.TLO.Navigation.Active() then
        ...
        mq.delay(2000)
        if stuckdistance < acleash then return false end
        if stuckdistance>=mq.TLO.Spawn(runconfig['followid']).Distance3D()+10 then stucktimer = mq.gettime() + 60000 return end
    ...
    function Wiggle(heading, size)
        ...
        if mq.TLO.Plugin("MQ2AutoSize").IsLoaded() then mq.cmdf('/squelch /multiline ; /face fast heading %s ; /stand ; /autosize sizeself %s ; /keypress forward hold', heading, size) end
        mq.delay(2000)
        if stuckdistance < acleash then return false end
        ...
        if mq.TLO.Spawn(runconfig['followid']).Distance3D() and stuckdistance>=mq.TLO.Spawn(runconfig['followid']).Distance3D()+10 then stucktimer = mq.gettime() + 60000 return true end
        mq.delay(100)
        if stuckdistance < acleash then return false end
        ...
        mq.cmdf('/squelch /nav id %s log=off', runconfig['followid'])
        if mq.TLO.Navigation.Active() then mq.delay(1000) end
        if followdist and stuckdistance and stuckdistance>=followdist+10 then stucktimer = mq.gettime() + 60000 return true end
    end
    ...
    if stuckdistance < acleash then return false end
    if followdist and stuckdistance >= followdist + 10 then stucktimer = mq.gettime() + 60000 return end
