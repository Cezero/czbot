...
function trotsbuff.LoadBuffConfig()
    ...
    for i=1, myconfig.buff['count'] do
        if i <= myconfig.buff['count'] then
            BuffClass[i] = {}
            buffkey = "ab"..i
            if not myconfig[buffkey] then myconfig[buffkey] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , spellicon = 0, precondition = true}  end
            if myconfig[buffkey]['gem'] == 'script' then
                if not myconfig['script'][myconfig[buffkey]['spell']] then 
                    ...
                    myconfig['script'][myconfig[buffkey]['spell']] = "test" end
...
function trotsbuff.BuffEval(index)
    ...
    local range = (mq.TLO.Spell(spell).MyRange() and mq.TLO.Spell(spell).MyRange()>0) and  mq.TLO.Spell(spell).MyRange() or mq.TLO.Spell(spell).AERange() or (gem == 'item' and mq.TLO.FindItem(spell).Spell.ID())
        ...
        if BuffClass[index]['self'] then
            ...
            if IconCheck(index, myid) then
                ...
                if (not mysong) or (mysongdur and mysongdur < 6100) then
                    if songtartype and (songtartype == 'Group v1' or songtartype == 'Group v2' or songtartype == 'Self' or songtartype == 'AE PC v2') then return 1, 'self'
