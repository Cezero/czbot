...
---@field ActorMaOverride table|nil manual ma_update: { name, seq, ts, zone?, publisher?, reason?, expiresAt? }
---@field ActorMtOverride table|nil manual mt_update: { name, seq, ts, zone?, publisher?, reason?, expiresAt? }
...
function M.canStartBusyState(stateNum)
    ...
    if stateNum == M.STATES.chchain then
        return current ~= M.STATES.dead
    end

    if stateNum == M.STATES.unstuck or stateNum == M.STATES.dragging or stateNum == M.STATES.raid_mechanic then
        return false
    end
