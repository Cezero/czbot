...
function trotscure.LoadCureConfig()
    ...
    for i=1, myconfig.cure['count'] do
        ...
        if i <= myconfig.cure['count'] then
            ...
            CureType[i] = {}
            curekey = "ac"..i
            if not myconfig[curekey] then myconfig[curekey] = {gem = 0, spell = 0, minmana = 0, alias = false, announce = false, curetype = "all", tarcnt = 0, class = 'war brd clr pal shd shm rng rog ber mnk dru bst mag nec enc wiz' , priority = false, precondition = true}  end
            if myconfig[curekey]['gem'] == 'script' then
                if not myconfig['script'][myconfig[curekey]['spell']] then 
                    ...
                    myconfig['script'][myconfig[curekey]['spell']] = "test" end
...
function trotscure.CureEval(index)
    ...
    if cureindex and cureindex['self'] then 
        for k, v in pairs(CureType[index]) do
            ...
            if
                string.lower(v) ~= 'all' and curetype and spelltartype == 'Self' then return 1, 'self'
            elseif
                string.lower(v) ~= 'all' and curetype then return mq.TLO.Me.ID(), 'self'
            end
        end
    end
                ...
                if cureindex[botclass] and botid then
                    if mq.TLO.Group.Member(botname).ID() then
                        for k,v in pairs(CureType[index]) do
                            ...
                            if string.lower(v) == "all" and botdtr and botdtr>0 and trotslib.DistanceCheck('ac', index, botid) then return botid, 'group' end
                            if string.lower(v) ~= 'all' and curetype and curetype > 0 then
            ...
            if botname then
                ...
                for k,v in pairs(CureType[index]) do
                    ...
                    if botclass and v and type(v) == "string" then
                        if botid and string.lower(v) == "all" and botdtr and botdtr>0 then return botid, botclass:lower() end
                        if string.lower(v) ~= 'all' and curetype and curetype > 0 then
